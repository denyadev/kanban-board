...
const initialTasks: Task[] = [
    { id: 1, title: "Create board layout", columnId: "todo", createdDate: new Date("2023-02-20"), color: 'bg-[#ff5c5c]'},
    { id: 2, title: "Implement drag and drop functionality", columnId: "todo", createdDate: new Date("2023-01-01"), color: 'bg-[#39da8a]' },
    { id: 3, title: "Implement the ability to add new tasks", columnId: "todo", createdDate: new Date("2023-02-05"), color: 'bg-[#5b8dee]' },
    { id: 4, title: "Implement task deletion", columnId: "todo", createdDate: new Date("2023-01-10"), color: 'bg-[#fdac41]' },
    { id: 5, title: "Add mobile responsiveness to the application", columnId: "inprogress", createdDate: new Date("2023-02-15"), color: 'bg-[#fedd49]' },
    { id: 6, title: "Add column editing form", columnId: "inprogress", createdDate: new Date("2023-01-20"), color: 'bg-[#74e0e6]' },
    { id: 7, title: "Add persistence using local storage", columnId: "complete", createdDate: new Date("2023-01-25"), color: 'bg-[#ac5cd9]' },
  ];
...
const KanbanBoard: React.FC = () => {
    ...
    const handleOnDrag = (e: React.DragEvent, taskId: number, columnId: string) => {
    ...
    const handleOrderSwap = (e: React.DragEvent, taskId: number, columnId: string) => {
        // Store the taskId and the columnId of our dragged task.
    ...
    const getRandomColor = (): string => {
        const colors = ["#ff5c5c", "#39da8a", "#5b8dee", "#fdac41", "#fedd49", "#74e0e6", "#ac5cd9"];
    ...
    return (
        <div className="text-white p-10 min-h-screen">
            <h1 className="text-4xl font-bold mb-10 text-center">
                <span className="text-[#19a35e]">K</span>anban <span className="text-[#19a35e]">B</span>oard <span className="text-base">by Denis</span>
            </h1>
            <div className="grid sm:grid-cols-1 md:grid-cols-3 gap-4">
                {columns.map(column => (
                    <section key={column.id} className="bg-[#28293d] rounded-sm"
                        // We attach handlers which represent the Column area
                        onDragOver={(e) => e.preventDefault()}
                        onDrop={(e) => handleOnColumnDrop(e, column.id)}>
                        <div className="bg-black/50 p-4 flex justify-between items-center rounded-sm uppercase">
                            <h2 className="font-bold">{column.title}</h2>
                            <button className="bg-[#19a35e] px-4 p-1 rounded-sm text-base" onClick={() => handleTaskAdd(column.id)}>Add</button>
                        </div>
                        <ul className="p-4 space-y-4">
                            {tasks.filter(task => task.columnId === column.id).map(task => (
                                <li key={task.id}
                                    ...
                                    draggable
                                    onDragStart={(e) => handleOnDrag(e, task.id, task.columnId)}
                                    onDrop={(e) => handleOrderSwap(e, task.id, task.columnId)}
                                    onDragOver={(e) => e.preventDefault()}
                                    >
